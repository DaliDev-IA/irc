...
void	Server::process_command(int fd, const ParsedCommand &command)
{
	...
	else if (ascii_case_equal(command.command, "NOTICE"))
		handle_notice(fd, command);
	...
	else if (ascii_case_equal(command.command, "WHO"))
		handle_who(fd, command);
	else if (ascii_case_equal(command.command, "QUIT"))
		handle_quit(fd, command);
	else if (!ascii_case_equal(command.command, "PONG"))
		send_unknown_command(fd, command.command);
}